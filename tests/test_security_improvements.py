...
class TestDebugWithExternalDB:
    """אזהרה כש-DEBUG=True עם DB שאינו localhost"""
...
class TestOTPHashing:
    """בדיקות שה-OTP נשמר כ-hash ולא כ-plaintext"""

    @pytest.mark.unit
    def test_hash_otp_returns_sha256(self):
        """_hash_otp מחזיר SHA-256 hex digest"""
        otp = "123456"
    ...
    async def test_otp_stored_as_hash_not_plaintext(self, fake_redis):
        """OTP נשמר ב-Redis כ-hash — לא כטקסט פשוט"""
        ...
        assert stored == hashlib.sha256("123456".encode()).hexdigest()
