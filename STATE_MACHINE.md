...
## מצבי נהג (DriverState)

> נהג (iDriver) יכול לחפש עבודה דרך הבוט: רישום, הגדרות סינון, חיפושים, פרסום נסיעות ומנוי.

### תיאור מצבי נהג

| מצב | תיאור | פעולה הבאה |
|-----|-------|-----------|
| `DRIVER.INITIAL` | מצב התחלתי | רישום / תפריט (נהג רשום) |
| `DRIVER.NEW` | נהג חדש | איסוף שם |
| `DRIVER.REGISTER.COLLECT_NAME` | איסוף שם מלא | תאריך לידה |
| `DRIVER.REGISTER.COLLECT_BIRTH_DATE` | תאריך לידה (dd/mm/yyyy) | תיאור רכב |
| `DRIVER.REGISTER.COLLECT_VEHICLE` | תיאור רכב חופשי | קוד לבוש |
| `DRIVER.REGISTER.COLLECT_DRESS_CODE` | בחירת קוד לבוש | אימות (חרדי) / תפריט (חילוני) |
| `DRIVER.VERIFY.COLLECT_SELFIE` | העלאת סלפי | תעודת זהות |
| `DRIVER.VERIFY.COLLECT_ID_DOCUMENT` | העלאת תעודת זהות | המתנה לאישור |
| `DRIVER.VERIFY.PENDING_APPROVAL` | ממתין לאישור מנהל | תפריט (לאחר אישור) |
| `DRIVER.MENU` | תפריט ראשי | הגדרות / חיפוש / מנוי |
| `DRIVER.SETTINGS.VIEW` | תפריט הגדרות חיפוש | בחירת הגדרה / חזרה |
| `DRIVER.SETTINGS.VEHICLE_TYPE` | שינוי סוג רכב | הגדרות / חזרה |
| `DRIVER.SETTINGS.TRIP_TYPE` | שינוי סוג נסיעה | הגדרות / חזרה |
| `DRIVER.SETTINGS.SHOW_DELIVERIES` | הצגת/הסתרת משלוחים | הגדרות / חזרה |
| `DRIVER.SETTINGS.UPCOMING_TIMEFRAME` | מסגרת זמן (שעה/שעתיים/5/הכל) | הגדרות / חזרה |
| `DRIVER.SETTINGS.FUTURE_ONLY_MODE` | חיפוש עתידי בלבד | שעת התחלה / הגדרות / חזרה |
| `DRIVER.SETTINGS.START_TIME` | הגדרת שעת התחלה | הגדרות / חזרה |
| `DRIVER.SEARCH.CREATE_ORIGIN` | שיתוף מיקום GPS למוצא | יעד / חזרה |
| `DRIVER.SEARCH.CREATE_DESTINATION` | הזנת עיר יעד | סוג חיפוש / חזרה |
| `DRIVER.SEARCH.CREATE_TYPE` | בחירת סוג חיפוש | חיפושים פעילים / חזרה |
| `DRIVER.SEARCH.VIEW_ACTIVE` | צפייה בחיפושים פעילים | ניהול / חיפוש חדש / חזרה |
| `DRIVER.SEARCH.MANAGE` | ניהול (מחיקת) חיפוש | חיפושים פעילים / חזרה |
| `DRIVER.SUBSCRIPTION.VIEW` | צפייה בסטטוס מנוי | רכישה / חזרה |
| `DRIVER.SUBSCRIPTION.PURCHASE` | רכישת מנוי | צפייה / חזרה |

### פקודות מהירות (מעובדות ב-MENU)
...
| `מחירון <מוצא> <יעד>` | הצגת מחירון למסלול |
| `בב <יעד> <מקומות> מק <מחיר>` | פרסום נסיעה |

### כללי רישום

- **זרם חילוני:** רישום (4 שלבים) → תפריט ישיר + מנוי ניסיון 7 ימים
- **זרם חרדי:** רישום (4 שלבים) → אימות (סלפי + ת.ז.) → המתנה לאישור מנהל → תפריט
- **בדיקת רישום קיים:** `_handle_initial` בודק `is_registration_complete` לפני התחלת רישום חדש
- **מגבלת חיפושים:** מקסימום 9 חיפושים פעילים לנהג
- **סשן 24 שעות:** סשן נפתח עם הודעה ראשונה, פג אחרי 24 שעות. חיפושים מושהים אוטומטית.
...
מצב נוכחי: MENU
הודעה: "בננה"
תגובה: "🤔 לא הבנתי. נסו שוב מהתפריט"
