...
4. אם לא נמצא — לא ניתן להבטיח שליחה אינטראקטיבית. ממשיכים לנסות לפתור LID→`@c.us` (למשל אחרי הודעה נוספת מהמשתמש) לפני ניסיון נוסף לשליחת תפריט.

### 3. נתיב retry ("No LID for user")

כשהשליחה ל-`@c.us` נכשלת עם `"No LID for user"`:
- המשתמש עבר ל-LID, ה-`@c.us` כבר לא תקין
- ה-retry בונה כתובת `@lid` ושולח הודעה שאינה אינטראקטיבית
...
4. **`"No LID for user"` = המשתמש עבר ל-LID** — צריך retry עם `@lid`, אבל לא לשלוח הודעות אינטראקטיביות
